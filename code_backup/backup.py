...
def save_output(t, save_img, content_image, iter_name, model_mean, no_num=False):
    ...
    disp = transforms.ToPILImage()(np.uint8(disp * 255))
...
def deprocess(output_tensor, mode='caffe', input_mean=[-103.939, -116.779, -123.68]):
    ...
    # turn tensor into image (c, h, w) -> (h, w, c)
    output_tensor.transpose_(0, 1).transpose_(1, 2)
    image = output_tensor.data.numpy()
    # save image as file 
